...
void TexWavesApp::BuildDescriptorHeaps()
{
	...
	auto grassTex = mTextures["waterTex"]->Resource;
	auto waterTex = mTextures["fenceTex"]->Resource;
	auto fenceTex = mTextures["grassTex"]->Resource;
...
void TexWavesApp::BuildRenderItems()
{
	...
	
	{
		...
		XMVECTOR axis = { 0.2f, 0.45f, 1.0f };
		XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(0.2f, 0.2f, 0.2f) * XMMatrixRotationAxis(axis, 20));
...

	{
		...
		XMVECTOR axis = { 0.2f, 0.45f, 1.0f };
		XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(0.2f, 0.2f, 0.2f) * XMMatrixRotationAxis(axis, 150) * XMMatrixTranslation(1, 3, 2));
		mRitemLayer[(int)RenderLayer::Opaque].push_back(boxRitem.get());
		mAllRitems.push_back(std::move(boxRitem));
	}

	{
		...
		XMVECTOR axis = { 0.2f, 0.45f, 1.0f };
		XMStoreFloat4x4(&boxRitem->World, XMMatrixScaling(0.5f, 0.5f, 0.5f) * XMMatrixRotationAxis(axis, 230)* XMMatrixTranslation(-2, 1, 3));
		mRitemLayer[(int)RenderLayer::Opaque].push_back(boxRitem.get());
		mAllRitems.push_back(std::move(boxRitem));
	}
